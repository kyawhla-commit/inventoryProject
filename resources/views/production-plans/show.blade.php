                            ...
                            <div class="col-md-3">
                                <div class="card border-0 bg-primary bg-opacity-10">
                                    <div class="card-body text-center">
                                        <h3 class="text-primary mb-1">{{ $productionPlan->productionPlanItems->count() }}
                                        </h3>
                                ...
                                <div class="card border-0 bg-success bg-opacity-10">
                                    <div class="card-body text-center">
                                        <h3 class="text-success mb-1">
                                            {{ $productionPlan->productionPlanItems->where('status', 'completed')->count() }}
                                        </h3>
                                ...
                                <div class="card border-0 bg-warning bg-opacity-10">
                                    <div class="card-body text-center">
                                        <h3 class="text-warning mb-1">
                                            {{ $productionPlan->productionPlanItems->where('status', 'in_progress')->count() }}
                                        </h3>
                                ...
                                <div class="card border-0 bg-info bg-opacity-10">
                                    <div class="card-body text-center">
                                        <h3 class="text-info mb-1">
                                            {{ $productionPlan->productionPlanItems->where('status', 'pending')->count() }}
                                        </h3>
                                        ...
                                        <div class="row text-center">
                                            <div class="col-md-4 border-end">
                                                ...
                                                <h4 class="text-primary">
                                                    ${{ number_format($productionPlan->total_estimated_cost, 2) }}</h4>
                                            </div>
                                            <div class="col-md-4 border-end">
                                                <label class="text-muted small mb-1">Actual Cost</label>
                                                <h4
                                                    class="{{ $productionPlan->total_actual_cost <= $productionPlan->total_estimated_cost ? 'text-success' : 'text-danger' }}">
                                            ...
                                            <div class="col-md-4">
                                                ...
                                                @php
                                                    $variance =
                                                        $productionPlan->total_actual_cost -
                                                        $productionPlan->total_estimated_cost;
                                        ...
                                        <tbody>
                                            @foreach ($productionPlan->productionPlanItems as $item)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <span
                                                            class="text-muted">{{ $item->recipe?->name ?? 'N/A' }}</span>
                                                    </td>
                                                    ...
                                                    <td class="text-center">
                                                        <span
                                                            class="fw-semibold {{ $item->actual_quantity >= $item->planned_quantity ? 'text-success' : 'text-warning' }}">
                                                    ...
                                                    <td class="text-end">
                                                        <span
                                                            class="text-muted">${{ number_format($item->estimated_material_cost, 2) }}</span>
                                                    </td>
                                                    <td class="text-end">
                                                        <span
                                                            class="{{ $item->actual_material_cost <= $item->estimated_material_cost ? 'text-success' : 'text-danger' }}">
                        ...
                        <div class="mt-4">
                            <div class="d-flex flex-wrap gap-2">
                                @if ($productionPlan->status === 'draft')
                                    <a href="{{ route('production-plans.edit', $productionPlan) }}"
                                        class="btn btn-warning">
                                        <i class="fas fa-edit me-1"></i> Edit Plan
                                    </a>
                                    <form action="{{ route('production-plans.approve', $productionPlan) }}"
                                ...
                                @if ($productionPlan->status === 'approved')
                                    <form action="{{ route('production-plans.start', $productionPlan) }}" method="POST"
                                        class="d-inline">
                                        @csrf
                                        @method('PATCH')
                                ...
                                @if ($productionPlan->status === 'in_progress')
                                    <form action="{{ route('production-plans.complete', $productionPlan) }}"
                                        method="POST" class="d-inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="btn btn-success">
                                ...
                                <a href="{{ route('production-plans.material-requirements', $productionPlan) }}"
                                    class="btn btn-info">
                                    ...
                                    <tbody>
                                        @foreach ($materialUsages as $usage)
                                            <tr>
                                                <td class="ps-4">
                                                    <small
                                                        class="text-muted">{{ $usage->usage_date->format('M d, Y') }}</small>
                                                    <br>
                                                    <small
                                                        class="text-muted">{{ $usage->usage_date->format('H:i') }}</small>
                                                </td>
                                                <td>
                                                    <div class="d-flex align-items-center">
                                                ...
                                                <td class="text-center">
                                                    <span
                                                        class="fw-semibold">{{ number_format($usage->quantity_used  , 2) }}</span>
                                                ...
                                                <td class="text-end">
                                                    <span
                                                        class="fw-semibold">${{ number_format($usage->total_cost, 2) }}</span>
                                                </td>
                                                <td>
                                                    <small
                                                        class="text-muted">{{ $usage->recordedBy->name ?? 'System' }}</small>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                                ...
                                                <td>
                                                    <div class="d-flex align-items-center">
                                                        ...
                                                        <div class="flex-grow-1">
                                                            <small
                                                                class="fw-semibold">{{ $requirement['raw_material']->name ?? 'N/A' }}</small>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td class="text-end">
                                                    <small
                                                        class="fw-semibold">{{ number_format($requirement['total_required'] ?? 0, 2) }}</small>
                                                    <br>
                                                    <small class="text-muted">{{ $requirement['unit'] }}</small>
                                                </td>
                                                <td class="text-end">
                                                    <small
                                                        class="text-success">${{ number_format($requirement['estimated_cost'] ?? 0, 2) }}</small>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                    ...
                                    <tbody>
                                        @foreach ($materialRequirements as $requirement)
                                            <tr>
                                                ...
                                                <td class="text-end">
                                                    @php
                                                        $rawMaterial = $requirement['raw_material'] ?? null;
                                                        $quantity = $rawMaterial ? $rawMaterial->quantity : 0;
...
=======
                                                    <small
                                                        class="{{ $quantity >= $totalRequired ? 'text-success' : 'text-danger' }}">
                                                        {{ number_format($quantity, 2) }}
                                                    </small>
                                                </td>
                                                <td class="text-center">
                                                    @if (isset($requirement['raw_material']) && isset($requirement['total_required']))
                                                        @if ($requirement['raw_material']->quantity >= $requirement['total_required'])
                                                            <i class="fas fa-check-circle text-success"
                                                                title="Sufficient"></i>
                                                        @else
                                                            <i class="fas fa-exclamation-triangle text-danger"
                                                                title="Insufficient"></i>
                                                        @endif
                                                    @else
                                                        <i class="fas fa-question-circle text-warning"
                                                            title="Data Missing"></i>
                                                    @endif
                                                </td>
                                            </tr>
                                        @endforeach
