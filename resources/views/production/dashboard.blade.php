                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @php
                                                    ...
                                                    $inProduction = $productsProduced
                                                        ->where('product.id', $product->id)
                                                        ->first()
                                                        ? $productsProduced->where('product.id', $product->id)->first()[
                                                            'total_produced'
                                                        ]
                                                        : 0;
                                                @endphp
                                ...
                                <tbody>
                                    @foreach ($criticalMaterials as $material)
                                        <tr>
                                            ...
                                            <td>{{ number_format($material->monthly_usage, 2) }} {{ $material->unit }}
                                            </td>
                                            <td>
                                                <span
                                                    class="badge bg-{{ $material->days_remaining < 7 ? 'danger' : 'warning' }}">
                                    ...
                                    <table class="table table-striped">
                                        ...
                                        <tbody>
                                            @foreach ($topProducts as $productData)
                                                <tr>
                                                    <td>{{ $productData['product']->name ?? 'N/A' }}</td>
                                                    <td>{{ $productData['total_produced'] ?? 0 }}
                                                        {{ $productData['product']->unit ?? '' }}</td>
                                                    <td>{{ $productData['current_stock'] ?? 0 }}
                                                        {{ $productData['product']->unit ?? '' }}</td>
                                                    <td>{{ config('app.currency_symbol', 'kyat') }}{{ number_format($productData['total_cost'] ?? 0, 2) }}</td>
                                                    <td>${{ number_format($productData['avg_cost_per_unit'] ?? 0, 2) }}
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                        ...
                                        <tbody>
                                            @foreach ($ordersFulfilled as $orderData)
                                                <tr>
                                                    ...
                                                    <td>{{ $orderData['fulfilled_items'] }}/{{ $orderData['total_items'] }}
                                                    </td>
                                                    <td>
                                                        <div class="progress">
                                                    ...
                                                    <td>
                                                        <span
                                                            class="badge bg-{{ $orderData['fulfillment_percentage'] == 100 ? 'success' : 'warning' }}">
